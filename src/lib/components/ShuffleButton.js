...
const ShuffleButton = props => {
  ...
  const enabled = !isShuffleLoading
    && ((props.librarySize && props.dbSize >= props.librarySize * .9) || props.isLoaded);
  ...
  const fillRandomPlaylist = playlist => {
    const trackCount = props.config.amountType == 'minutes'
        ? Math.round(props.config.trackMinutes / 2)
    ...
    const addTracks = numbers => {
      db.tracks.toArray(library => {
        ...
        slices.forEach(chunk => {
          if (props.config.amountType == 'minutes'
            && minutes >= props.config.trackMinutes) {
            return;
          }
      ...
      preparePurge(playlist.tracks.href).then(trackUris => {
          ...
          const removeChunk = chunks => {
              return new Promise((resolve, reject) => {
                  ...
                  purgePlaylistTracks(playlist, chunk)
                      .then(_ => {
  ...
  return (
    <button className="btn btn-primary" disabled={!enabled} onClick={startShuffle}><i className={icon} />&nbsp;Shuffle</button>
  );
}
...
function mapStateToProps({data, auth}) {
  return {
    ...
    dbSize: data.dbSize,
    authenticated: auth,
    existingPlaylist:  data.playlists ? data.playlists.reduce((accumulator, currentValue) => {
